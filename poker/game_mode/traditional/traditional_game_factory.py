...
# @Author : lxf 
# @Version：V 0.1
# @File : traditional_game_factory.py
# @desc :
import uuid
from typing import Optional, List
from poker.game_core.deck.deck import DeckFactory
...
from poker.game_core.room.game_room import GameSubscriber
from poker.game_core.scoring.holdem_score_detector import HoldemPokerScoreDetector
from poker.game_mode.traditional.traditional_event_dispatcher import HoldemPokerGameEventDispatcher
from poker.game_mode.traditional.traditional_game import HoldemPokerGame
...
class HoldemPokerGameFactory(GameFactory):
    ...
    def create_game(self, players: List[Player]):
        # 修改player的传入类型可以为PlayerServer类的父类Player
        ...
        return HoldemPokerGame(
            ...
            self._small_blind,
            id=game_id,
            game_players=GamePlayers(players),  # 游戏中的玩家管理
            event_dispatcher=event_dispatcher,  # 游戏事件分发
            deck_factory=DeckFactory(lowest_rank=2),  # 发牌器
            score_detector=HoldemPokerScoreDetector()  # 游戏规则判定，我的规矩就是规矩
        )
