...
class HoldemPokerScoreDetectorTests(unittest.TestCase):
    ...
    def test_bad_case(self):
        # 777555葫芦被识别成三条的问题，已解决
        board = [Card(7, 0), Card(7, 1), Card(7, 3), Card(6, 0), Card(13, 0)]
        player1 = [Card(6, 3), Card(6, 1)]
        player2 = [Card(12, 2), Card(8, 0)]
        sd = HoldemPokerScoreDetector()
        score1 = sd.get_score(player1 + board)
        score2 = sd.get_score(player2 + board)
        self.assertEquals(1, score1.cmp(score2))
